...
(현재 NEIS는 정해진 neis_000000.xlsx 형식만 업로드 가능하므로,
 완전 자동화보다는 '템플릿에 붙여넣기 쉬운 형태'를 목표로 합니다.)

데이터 의존성:
  - 학기: AcademicTerm
  - 반별 출력: SchoolClass + TimetableEntry (GenerateWorker 로 생성)
  - 교사별 출력: Teacher + TimetableEntry
  - 다이얼로그가 열릴 때마다 DB 에서 최신 데이터를 조회합니다.

시트 관리:
  반별 출력: 각 반마다 별도 시트 (시트명 = display_name, 예: "1-1")
  교사별 출력: 각 교사마다 별도 시트 (시트명 = 교사 이름, 최대 20자)
  첫 번째 항목은 기본 시트("Sheet")를 재사용하고, 두 번째부터 create_sheet()

  중요: 시트명은 Excel 제한으로 31자까지 가능하나, 특수문자(\ / ? * [ ] :)는
        사용할 수 없습니다. 교사명이 20자를 초과하면 앞 20자만 사용합니다.

각 시트 구조:
  1행: 타이틀 (A1~F1 병합 셀, 굵은 남색 14pt)
  2행: 비어 있음 (시각적 여백)
  3행: 헤더 (A3="", B3~F3=요일. 남색 배경 + 흰색 글자)
  4행~: 교시별 수업 데이터 (A열=교시 레이블, B~F열=교과약어+교사명)

스타일 상수는 시트 생성마다 재사용되므로 함수 외부에 정의되어 있습니다.
셀 스타일링: 남색 헤더(#1B4F8A), 회색 교시 레이블(#E8ECF0), 얇은 테두리,
            모든 텍스트는 가운데 정렬 + 자동 줄바꿈.
"""

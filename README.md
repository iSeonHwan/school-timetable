...
4. [상세 사용 설명](#4-상세-사용-설명)
5. [프로젝트 구조](#5-프로젝트-구조)
6. [데이터베이스 구조 (ERD)](#6-데이터베이스-구조-erd)
7. [시간표 자동 생성 알고리즘](#7-시간표-자동-생성-알고리즘)
8. [테스트 실행](#8-테스트-실행)
9. [설치 프로그램 빌드](#9-설치-프로그램-빌드)
10. [기술 스택](#10-기술-스택)
11. [장점과 한계](#11-장점과-한계)
...
알고리즘의 자세한 동작 방식은 [7장 시간표 자동 생성 알고리즘](#7-시간표-자동-생성-알고리즘)에서 설명합니다.
...
현재 테스트 파일은 `tests/test_feedback.py`에 피드백 다이얼로그 테스트가 포함되어 있습니다. 이 테스트는 임시 디렉터리에 `feedback.json`을 생성하여 실제 파일 저장·읽기 동작을 검증합니다.
